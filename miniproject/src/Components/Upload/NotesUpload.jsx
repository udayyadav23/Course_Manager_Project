  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8082/uploads/notes/${courseName}`,
        formData,
      ...
      if (res.status === 202) {
        ...
        setNoteIds(courseRes.data.notes);
        setFile(null);
      } else {
  ...
  return (
    <div className="assignment-upload-container">
      <div className="upload-card">
        ...
        <button onClick={handleUpload} className="upload-btn">Add Note</button>
      ...
      <div className="assignment-gallery">
        ...
        {noteIds.length === 0 ? (
          <p className="no-assignments">No notes uploaded yet.</p>
        ) : (
